...
public class LynxCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            switch (args[0].toLowerCase()) {
                ...
                case "delete":
                    if (args.length < 2) {
                        player.sendMessage(ChatColor.RED + "Usage: /lynx delete minigame <name>");
                        return true;
                    }
                    if (args[1].equalsIgnoreCase("minigame")) {
                        handleDeleteMinigame(player, args);
                    } else {
                        player.sendMessage(ChatColor.RED + "Invalid delete command. Use: /lynx delete minigame <name>");
                    }
                    break;
                case "friends":
    ...
    private void handleServerSignCommand(Player player, String[] args) {
        ...
        switch (args[1].toLowerCase()) {
            case "create":
                ...
                // TODO: Implement server sign creation
                player.sendMessage(ChatColor.RED + "Server sign creation is not implemented yet.");
                break;
            case "remove":
                // TODO: Implement server sign removal
                player.sendMessage(ChatColor.RED + "Server sign removal is not implemented yet.");
                break;
            default:
    ...
    private void handleDeleteMinigame(Player player, String[] args) {
        if (!player.hasPermission("lynx.deleteminigame")) {
            player.sendMessage(ChatColor.RED + "You don't have permission to delete minigames.");
            return;
        }
        ...
        if (!configManager.getMinigames().contains(minigameName)) {
            player.sendMessage(ChatColor.RED + "Minigame '" + minigameName + "' does not exist!");
            return;
        }

        // Delete the minigame
        configManager.deleteMinigame(minigameName);
        player.sendMessage(ChatColor.GREEN + "Successfully deleted minigame '" + minigameName + "'!");

        // Reload the navigator to reflect changes
        plugin.reloadNavigator();
    }
    ...
    private void handleFriendsCommand(Player player, String[] args) {
        ...
        switch (args[1].toLowerCase()) {
            ...
            case "accept":
                ...
                friendManager.acceptFriendRequest(player.getName(), args[2]);
    ...
    private void handlePrivacyCommand(Player player, String[] args) {
        ...
        try {
            PrivacySettings level = PrivacySettings.valueOf(args[1].toUpperCase());
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            completions.addAll(Arrays.asList(
                    ...
                    "serversign", "help", "friends", "privacy","delete"
            ));
        ...
        } else if (args.length == 3 && args[0].equalsIgnoreCase("delete") && args[1].equalsIgnoreCase("minigame")) {
