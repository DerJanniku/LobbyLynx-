...
public class FriendCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleGroupCommand(Player player, String[] args) {
        ...
        switch (action.toLowerCase()) {
            ...
            case "remove":
                ...
                break;
            case "list":
                List<String> groupMembers = friendManager.getFriendsInGroup(player.getName(), groupName);
    ...
    private void handleFavoriteCommand(Player player, String[] args) {
        ...
        String targetName = args[1];
        friendManager.toggleFavoriteFriend(player.getName(), targetName);
    }
    ...
    private void handleGiftCommand(Player player, String[] args) {
        ...
        String targetName = args[1];
        String giftType = args[2];
        friendManager.sendFriendGift(player.getName(), targetName, giftType);
    }
    ...
    private void handleChallengeCommand(Player player, String[] args) {
        ...
        String targetName = args[1];
        String challengeType = args[2];
        friendManager.createFriendChallenge(player.getName(), targetName, challengeType);
    }
    ...
    private void handleSearchCommand(Player player, String[] args) {
        ...
        String query = args[1];
        List<String> results = friendManager.searchFriends(player.getName(), query);
    ...
    // Utility method to check if a player is online
    private boolean isPlayerOnline(String playerName) {
        return Bukkit.getPlayer(playerName) != null;
    }

    // Utility method to get online status as a colored string
    private String getOnlineStatus(String playerName) {
        return isPlayerOnline(playerName) ? ChatColor.GREEN + "Online" : ChatColor.RED + "Offline";
    }
    ...
    // Utility method to send a paginated message
    ...
    // Utility method to validate and parse integer input
    ...
    // Utility method to format a list of strings for display
    ...
    // Utility method to check if two players are mutual friends
    ...
    // Utility method to get a player's blocked players count
    private int getBlockedCount(String playerName) {
        return friendManager.getBlockedPlayers(playerName).size();
    }

    // Utility method to check if a player has reached the maximum number of friends
    ...
    // Utility method to send a friend request cooldown message
